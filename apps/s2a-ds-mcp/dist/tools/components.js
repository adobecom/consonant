...
function findComponent(name, components) {
    ...
    return components.find((c) => normalizeComponentName(c.name) === q ||
        normalizeComponentName(c.slug) === q ||
        normalizeComponentName(c.cssClass) === q);
}
export function registerComponentTools(server, dsRoot) {
    // ── get_component ─────────────────────────────────────────────────────────
    server.tool("get_component", "Get full schema for a component — props, CSS tokens used, CSS class, Figma node ID, and source file paths. Use the component name (e.g. 'RouterCard', 'router-card') or CSS class ('c-router-card').", {
    ...
    // ── list_components ────────────────────────────────────────────────────────
    server.tool("list_components", "List all components in the design system with summary info — name, CSS class, prop count, token count, and Figma node ID.", {
    ...
    server.tool("find_component_for_use_case", "Given a description of what you're building, find the best matching component(s). Returns ranked matches with confidence and reasoning.", {
    ...
    server.tool("get_component_tokens", "Get all design tokens used by a component, with their resolved values across all modes.", {
