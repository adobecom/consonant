 ...
 * Excludes grid/list containers whose name contains "card"/"tile" but whose
 * children are themselves individual cards (e.g. "Product Cards" wrapper).
 */
function isCardOrTile(node: SceneNode): boolean {
  ...
  // If this node contains 3+ card-sized children with card/tile names,
  // it's a grid container — not an individual card.
  if ('children' in node) {
    const cardChildren = (node as FrameNode).children.filter(c => {
      const cName = c.name.toLowerCase();
      return c.visible && (cName.includes('card') || cName.includes('tile')) &&
 ...
 *    — detects "Home", "Products", "About Us" etc. inside top nav bars
 * 3. Child of a nav-like VERTICAL container with 3+ similarly-sized siblings
 *    — detects sidebar tabs, filter lists, vertical navigation
...
function isNavItem(node: SceneNode): boolean {
  ...
  // Strategy 2: sibling similarity inside a HORIZONTAL container (top nav bars)
  ...
  // Strategy 3: sibling similarity inside a VERTICAL container (sidebar tabs, filter lists)
  // Use height (not width) as the consistency signal — vertical tab items share a row height
  // but have variable widths because text labels differ in length.
  if (container.layoutMode === 'VERTICAL') {
    const heights = [...siblings.map(s => s.height)].sort((a, b) => a - b);
    const medianH = heights[Math.floor(heights.length / 2)];
    // Height should be consistent across siblings (within 40%)
    if (Math.abs(node.height - medianH) > medianH * 0.4) return false;
    // Tab items are compact rows — not taller than 80px
...
function collectFocusable(node: SceneNode, results: SceneNode[], depth = 0): void {
  ...
  for (const child of container.children) {
    ...
    // Skip annotation frames generated by this plugin — their internal shapes match
    // interactive keywords but are not real interactive elements.
    const cNameLower = child.name.toLowerCase();
    if (
      cNameLower === 'accessibility annotations' ||
      cNameLower.startsWith('a11y annotations') ||
      cNameLower === 'blueline cards' ||
      cNameLower === 'tier 2 cards'
    ) continue;
