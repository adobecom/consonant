...
- `s2a-audit.ts`
- `tokens.ts` exports (`S2A_COLLECTION_KEYS`, `loadLibraryTokens`, `isLoaded`, `colorVarMap`, `dimensionVarMap`, `textStyleMap`, `lookupTextStyleById`, `matchColor`, `matchRadius`, `matchSpacing`, `matchDimension`, `matchTypographyStrict`, `detectNodeColorRole`)
...
import { isLoaded, loadLibraryTokens, colorVarMap, dimensionVarMap, textStyleMap, lookupTextStyleById, matchTypographyStrict, detectNodeColorRole } from './tokens';
...
async function auditColorPaint(
  ...
  if (boundId) {
    ...
    const matches = colorVarMap.filter(cv => cv.hex.toLowerCase() === hex.toLowerCase());
...
async function classifyDim(
  ...
  if (boundId) {
    ...
    const exact = dimensionVarMap.find(v =>
...
export async function auditTypography(node: SceneNode): Promise<AlignV2Issue[]> {
  ...
  if (styleId && styleId !== '' && styleId !== figma.mixed) {
    ...
    const suggestion = matchResult.matched
      ? { tokenName: matchResult.name, textStyleId: textStyleMap.find(ts => ts.name === matchResult.name)?.styleId, isExactMatch: true }
      : null;
    return [{
      nodeId: node.id,
      nodeName: node.name,
