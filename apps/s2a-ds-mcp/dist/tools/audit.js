...
function fail(msg) {
    return { content: [{ type: "text", text: JSON.stringify({ success: false, error: "internal_error", message: msg }, null, 2) }], isError: true };
}
...
function parseColor(val) {
    ...
    if (t === "black" || t === "#000" || t === "#000000")
        return { r: 0, g: 0, b: 0, a: 1 };
    if (t === "white" || t === "#fff" || t === "#ffffff")
...
function colorNamespace(property) {
    const p = property.toLowerCase();
    if (TEXT_PROPS.has(p))
        return "content";
    if (BG_PROPS.has(p) || p.startsWith("background"))
        return "background";
    if (BORDER_COLOR_PROPS.has(p))
        return "border";
    if (p.includes("shadow") || p.includes("box-shadow"))
        return "transparent";
    if (p.includes("color"))
        return "any";
    return "any";
...
function parseCSSFile(css) {
    ...
    for (const m of css.matchAll(re)) {
        ...
        if (rgba) {
            ...
            if (varMatch)
                refs.set(prop, varMatch[1]);
        }
    }
    return { direct, refs };
}
function buildColorMap(dsRoot, index) {
    ...
    if (existsSync(cssDir)) {
        ...
        const resolved = new Map(directMap);
    ...
    for (const entry of index.all) {
        if (entry.type !== "color")
            continue;
        if (!LIGHT_MODES.has(entry.mode))
            continue;
        const rgba = parseColor(String(entry.rawValue));
        if (rgba && !map.has(entry.cssProp))
            map.set(entry.cssProp, rgba);
    }
    return map;
}
...
function isDimensionProp(property) {
    const p = property.toLowerCase();
...
function dimensionNS(property) {
    const p = property.toLowerCase();
    if (SPACING_PROPS.has(p))
        return "spacing";
    if (RADIUS_PROPS.has(p))
        return "border-radius";
    if (BORDER_WIDTH_PROPS.has(p))
        return "border-width";
    if (FONT_SIZE_PROPS.has(p))
        return "font-size";
    if (LINE_HEIGHT_PROPS.has(p))
        return "line-height";
    if (LETTER_SPACING_PROPS.has(p))
        return "letter-spacing";
    if (FONT_WEIGHT_PROPS.has(p))
        return "font-weight";
    if (FONT_FAMILY_PROPS.has(p))
        return "font-family";
    if (BLUR_PROPS.has(p))
        return "blur";
    return null;
...
function parseDimension(val) {
    ...
    if (/^(auto|inherit|initial|unset|none|normal|transparent|fit-content|max-content|min-content)$/.test(v))
        return null;
    if (/^\d+%$/.test(v))
        return null; // percentages are not tokenizable
    if (/^calc\(/.test(v))
        return null; // calc() expressions are not tokenizable
    if (/^var\(/.test(v))
        return null; // already using a token
...
function buildDimensionMap(dsRoot) {
    const cssDir = resolve(dsRoot, "dist/packages/tokens/css/dev");
    const map = new Map();
    const allRefs = new Map();
    if (!existsSync(cssDir))
...
function buildFontFamilyMap(dsRoot) {
    // Maps a font family string like "adobe-clean" to a list of token CSS props
    const familyToTokens = new Map();
    const cssDir = resolve(dsRoot, "dist/packages/tokens/css/dev");
    if (!existsSync(cssDir))
        return familyToTokens;
    const tokenToFamily = new Map(); // cssProp → resolved family string
    const refs = new Map();
...
function matchesDimNS(cssProp, ns) {
    switch (ns) {
...
function findClosestDimension(targetPx, ns, dimensionMap, index, topN = 3) {
    const candidates = [];
    const seen = new Set();
    for (const [cssProp, tokenVal] of dimensionMap) {
        if (!matchesDimNS(cssProp, ns))
            continue;
        if (seen.has(cssProp))
            continue;
        seen.add(cssProp);
...
function findClosest(target, ns, index, colorMap, topN = 3) {
    const seen = new Set();
    const candidates = [];
    for (const entry of index.all) {
        if (entry.designOnly)
        ...
        if (seen.has(entry.cssProp))
            continue;
        if (!isPrimitive(entry) && !isSemantic(entry))
            continue;
        if (!matchesColorNS(entry.cssProp, ns))
            continue;
...
function suggestNewColor(rgba, property) {
...
// ── Tool registration ─────────────────────────────────────────────────────────
export function registerAuditTools(server, dsRoot) {
    server.tool("audit_css", [
        ...
        try {
            ...
            const activeCategories = new Set(categories ?? ["color", "spacing", "border-radius", "border-width", "font-size", "line-height", "letter-spacing", "font-weight", "font-family", "blur"]);
            const violations = [];
            const seen = new Set();
            for (const decl of decls) {
                ...
                if (!isDimensionProp(property))
                    continue;
                const ns = dimensionNS(property);
                if (!ns || !activeCategories.has(ns))
                    continue;
                    ...
                    for (const cssProp of varRefs) {
                        ...
                        if (!entries || entries.length === 0) {
                            ...
                            if (!seen.has(dKey)) {
                                ...
                                violations.push({
                                    line, selector, property, value: `var(${cssProp})`,
                                    category: ns, violationType: "undefined-token", severity: "medium",
                                    resolution: { type: "suggest-new", confidence: "low", reasoning: `"${cssProp}" does not exist in the current token set` },
                                });
                            }
                        }
                            ...
                            if (primVal !== undefined) {
                                ...
                                if (semanticMatches.length > 0) {
                                    ...
                                    if (!seen.has(dKey)) {
                                        ...
                                        violations.push({
                                            line, selector, property, value: `var(${cssProp})`,
                                            category: ns, violationType: "primitive-dimension", severity: "medium",
                ...
                const numericVals = pxValues.length > 0 ? pxValues : (singleNum !== null ? [singleNum] : []);
                for (const targetPx of numericVals) {
                    if (targetPx === 0)
                        continue; // 0 = --s2a-spacing-0, rarely tokenized in practice
                    ...
                    const best = matches[0];
                    if (!best)
                        continue; // No token exists in this namespace — not flaggable
                    violations.push({
                        line, selector,
                        property,
            ...
            const bySeverity = (s) => violations.filter(v => v.severity === s).length;
            const byCategory = (c) => violations.filter(v => v.category === c).length;
            return ok({
                success: true,
