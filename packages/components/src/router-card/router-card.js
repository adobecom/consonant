...
const RouterCardCTA = () => html`
  <span class="c-router-card__action c-router-card__action--button" aria-hidden="true">
...
export const RouterCard = ({
  ...
  const headerAction = hasCustomAction
    ? html`<span class="c-router-card__action" aria-label=${actionLabel ?? nothing}>${actionTemplate}</span>`
    : normalizedState === "mobile"
      ? RouterCardCTA()
      : caretVisible
        ? html`<span class="c-router-card__action c-router-card__action--caret" aria-hidden="true">
            ${RouterCardCaret()}
          </span>`
        : nothing;
  ...
  const fallbackMedia = mediaSrc
    ? undefined
    : html`<span class="c-router-card__media-placeholder" aria-hidden="true"></span>`;
  ...
  const bodyContent = bodyTemplate ?? html`
    <p class="c-router-card__title">${title}</p>
    ${body ? html`<p class="c-router-card__body-text">${body}</p>` : nothing}
    ${children ? html`<div class="c-router-card__extra">${children}</div>` : nothing}
  `;

  const cardContent = html`
    <div class="c-router-card__header">
      ${ProductLockup(lockupProps)}
      ${headerAction}
    </div>
    <div class="c-router-card__media">${mediaNode}</div>
    <div class="c-router-card__body">
      <div class="c-router-card__body-content">
        ${bodyContent}
      </div>
      ${bodyAction ? html`<div class="c-router-card__body-action">${bodyAction}</div>` : nothing}
    </div>
  `;
