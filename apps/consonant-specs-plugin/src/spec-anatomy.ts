...
export async function collectSignificantNodes(node: SceneNode, entries: AnatomyEntry[], seen: Set<string>, depth: number = 0, includeMedia: boolean = false): Promise<void> {
  if (depth > 0) {
    ...
    if (!seen.has(key)) {
      ...
      if (node.type === 'TEXT') {
        ...
        // Media nodes: frames/rectangles with image fills, vectors, shapes
        const w = (node as any).width ?? 0;
        const h = (node as any).height ?? 0;
        const hasImage = 'fills' in node && Array.isArray((node as any).fills) &&
          ((node as any).fills as Paint[]).some(f => f.type === 'IMAGE');
...
export async function isCallToAction(node: SceneNode): Promise<boolean> {
