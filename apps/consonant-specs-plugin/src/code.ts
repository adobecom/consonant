...
import { runS2AAudit, runFullAlign, runTextColorsAlign, runSpacingAlign } from './s2a-audit';
...
figma.ui.onmessage = async (msg: { type: string; [key: string]: unknown }) => {
  switch (msg.type) {
    ...
    case 'full-align-s2a': {
      ...
      try {
        const result = await runFullAlign(sel[0], !!msg.skipSizing);
    ...
    case 'text-colors-align': {
      ...
      try {
        const result = await runTextColorsAlign(sel[0], !!msg.skipSizing);
    ...
    case 'spacing-align': {
      ...
      try {
        const result = await runSpacingAlign(sel[0], !!msg.skipSizing);
        figma.notify(`Spacing: ${result.aligned} bound to S2A, ${result.unmatched.length} unmatched (mode: ${result.mode})`);
        figma.ui.postMessage({ type: 's2a-align-result', ...result });
      } catch (e: any) {
    ...
    case 'force-match': {
      ...
      try {
        const result = await forceMatch(sel[0], categories, !!msg.skipSizing);
    ...
    case 'clear-annotations': {
      ...
      if (sel.length === 0) { figma.notify('Select a node first'); break; }
      let cleared = 0;
      function clearRecursive(node: BaseNode): void {
        if ('annotations' in node && (node as any).annotations?.length > 0) {
          (node as any).annotations = [];
          cleared++;
        }
        if ('children' in node) {
          for (const child of (node as any).children) clearRecursive(child);
        }
      }
      clearRecursive(sel[0]);
