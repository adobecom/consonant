...
export default {
  ...
  argTypes: {
    ...
    surface: {
      ...
      description: 'Surface context (default or on-media for buttons over images)',
    },
  },
  args: { 
    onClick: fn(),
...
export const AllVariants = {
  render: () => {
    const variants = [
      { kind: 'accent', background: 'solid', state: 'default', label: 'Accent Solid' },
      { kind: 'accent', background: 'solid', state: 'disabled', label: 'Accent Solid Disabled' },
      { kind: 'primary', background: 'outlined', state: 'default', label: 'Primary Outlined' },
      { kind: 'primary', background: 'outlined', state: 'disabled', label: 'Primary Outlined Disabled' },
      { kind: 'secondary', background: 'solid', state: 'default', label: 'Secondary Solid' },
      { kind: 'secondary', background: 'solid', state: 'disabled', label: 'Secondary Solid Disabled' },
      { kind: 'secondary', background: 'outlined', state: 'default', label: 'Secondary Outlined' },
      { kind: 'secondary', background: 'outlined', state: 'disabled', label: 'Secondary Outlined Disabled' },
    ];
...
export const OnMediaVariants = {
  render: () => {
    const variants = [
      { kind: 'primary', background: 'solid', surface: 'on-media', size: 'lg', state: 'default', label: 'Primary Solid' },
      { kind: 'secondary', background: 'glass', surface: 'on-media', size: 'lg', state: 'default', label: 'Secondary Glass' },
    ];

    return html`
      <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 40px; display: flex; flex-wrap: wrap; gap: 16px; align-items: center; justify-content: center; min-height: 200px;">
        ${variants.map(variant => Button(variant))}
      </div>
    `;
  },
      ...
      <div style="display: flex; flex-direction: column; gap: 24px; padding: 20px;">
        ${sizes.map(size => html`
          <div style="display: flex; align-items: center; gap: 16px;">
            ...
            ${Button({ kind: 'accent', background: 'solid', state: 'default', size, label: 'Label' })}
            ${Button({ kind: 'primary', background: 'outlined', state: 'default', size, label: 'Label' })}
            ${Button({ kind: 'secondary', background: 'solid', state: 'default', size, label: 'Label' })}
          </div>
        `)}
      </div>
    `;
