...
function removeOverlays(node: SceneNode, overlayName: string): void {
  ...
  // Also remove sibling overlays (placed as siblings to avoid clipping)
  if (node.parent && 'children' in node.parent) {
    const siblings = (node.parent as FrameNode).children.filter(c => c.name === overlayName);
...
export async function generateSpacingSection(sourceNode: SceneNode): Promise<void> {
  ...
  // Overlay as a sibling — not a child — so sourceNode's clipsContent
  // can't clip badges that extend past the frame edges.
  const overlay = figma.createFrame();
  overlay.name = 'spacing-detailed-overlay';
  overlay.fills = [];
  overlay.clipsContent = false;
  (parent as FrameNode).appendChild(overlay);
