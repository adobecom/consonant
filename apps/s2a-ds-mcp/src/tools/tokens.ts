...
import type { ResolvedToken, ToolError } from "../types.js";
...
export function registerTokenTools(server: McpServer, dsRoot: string): void {

  // ── resolve_token ───────────────────────────────────────────────────────
  server.tool(
    "resolve_token",
    "Resolve a design token by its display path (e.g. 's2a/color/background/default') or CSS variable name (e.g. '--s2a-color-background-default') to its values across all modes.",
  ...
  server.tool(
    "search_tokens",
    "Fuzzy search across token paths, CSS variable names, descriptions, and raw values. Returns ranked results.",
    {
    ...
    async ({ query, type, collection, limit, includeDesignOnly }) => {
      try {
        ...
        for (const entry of index.all) {
          ...
          // Filters
          if (!includeDesignOnly && entry.designOnly) continue;
  ...
  server.tool(
    "get_token_collection",
    "Return all tokens in a specific collection with their full values. Use list_token_collections to see available collection slugs.",
    {
  ...
  server.tool(
    "check_token_exists",
    "Check whether a token exists and whether it ships in CSS output (i.e. is not design-only).",
    {
      token: z.string().describe("Token display path or CSS custom property name"),
    },
    async ({ token }) => {
  ...
  server.tool(
    "get_token_aliases",
    "Find all semantic tokens that reference a given primitive value in their $value field. Useful for understanding the impact of changing a primitive.",
    {
  ...
  server.tool(
    "list_token_collections",
    "List all available token collections with their slugs, human names, modes, and token counts.",
    {},
    async () => {
      try {
        const index = loadTokens(dsRoot);
        const stats = new Map<string, { name: string; modes: Set<string>; count: number }>();
        ...
        const propsByCollection = new Map<string, Set<string>>();
        for (const entry of index.all) {
