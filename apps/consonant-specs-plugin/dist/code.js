...
async function forceMatch(node, categories, skipSizing = false) {
  if (!skipSizing) {
    const topFrame = findTopLevelFrame(node);
    const modeTarget = topFrame || ("layoutMode" in node ? node : null);
    if (modeTarget) {
      await setResponsiveMode(modeTarget);
    }
  }
...
async function collectSignificantNodes(node, entries, seen, depth = 0, includeMedia = false) {
  var _a, _b, _c, _d;
  if (depth > 0) {
    ...
    if (!seen.has(key)) {
      ...
      if (node.type === "TEXT") {
        ...
        const w = (_c = node.width) != null ? _c : 0;
        const h = (_d = node.height) != null ? _d : 0;
        const hasImage = "fills" in node && Array.isArray(node.fills) && node.fills.some((f) => f.type === "IMAGE");
        const MEDIA_PATTERNS = /\b(icon|image|img|media|photo|asset|illustration|logo|avatar|thumbnail|badge|chevron|arrow|video|play|pause|progress|divider|separator|line|rule)\b/i;
        const nameMatch = MEDIA_PATTERNS.test(node.name);
        if (hasImage && w >= 16 && h >= 16) {
          isSignificant = true;
        } else if (nameMatch && w >= 12 && h >= 12) {
          isSignificant = true;
        } else if ((node.type === "VECTOR" || node.type === "BOOLEAN_OPERATION") && w >= 12 && h >= 12 && w <= 200 && h <= 200) {
          isSignificant = true;
        } else if (node.type === "LINE" && w >= 20) {
          isSignificant = true;
        }
      }
      if (isSignificant) {
  ...
  if ("children" in node && depth < 10) {
    for (const child of node.children) {
      ...
      await collectSignificantNodes(child, entries, seen, depth + 1, includeMedia);
    }
  }
}
...
function removeOverlays(node, overlayName) {
  ...
  if (node.parent && "children" in node.parent) {
    const siblings = node.parent.children.filter((c) => c.name === overlayName);
...
async function generateSpacingSection(sourceNode) {
  ...
  overlay.x = sourceNode.x;
  overlay.y = sourceNode.y;
...
async function generateColorAnnotations(node, yOffset = 0) {
  ...
  async function walk(n, isRoot = false) {
    ...
    const skipAnnotation = hasImageFill3(n);
    if (!skipAnnotation) {
      const properties = [];
...
// src/spec-anatomy-annotations.ts
var CATEGORY_DEFS = [
...
var ICON_IMAGE_PATTERNS = /\b(icon|image|img|media|photo|asset|illustration|logo|avatar|thumbnail|badge|chevron|arrow)\b/i;
...
function describeTextRole(node) {
...
var MEDIA_NAME_PATTERNS = /\b(icon|image|img|media|photo|asset|illustration|logo|avatar|thumbnail|badge|chevron|arrow|video|play|pause|progress|divider|separator|line|rule)\b/i;
async function classifyNode(node) {
  ...
  if (node.type === "TEXT") {
    return { category: "Text", role: describeTextRole(node) };
  }
...
async function generateAnatomyAnnotations(sourceNode) {
...
async function specIt(node, sections = ["anatomy", "layout", "typography", "components"]) {
  ...
  if (enabled.has("anatomyAnnotation")) {
    figma.ui.postMessage({ type: "spec-it-status", message: "Adding anatomy annotations..." });
    const count = await generateAnatomyAnnotations(node);
    figma.ui.postMessage({ type: "spec-it-status", message: `${count} anatomy annotations added` });
  }
...
async function alignNode(node, textOnly, result2, spacingOnly = false) {
  var _a, _b;
  if (!spacingOnly && node.type === "TEXT") {
    const textNode = node;
  ...
  if (!textOnly && !spacingOnly && "fills" in node && Array.isArray(node.fills)) {
    const fills = node.fills;
  ...
  if (!textOnly && !spacingOnly && "strokes" in node && Array.isArray(node.strokes)) {
    const strokes = node.strokes;
  ...
  if (!textOnly || spacingOnly) {
    if (!spacingOnly && "cornerRadius" in node && typeof node.cornerRadius === "number" && node.cornerRadius > 0) {
      result2.scanned++;
    ...
    if (!spacingOnly && "strokeWeight" in node && typeof node.strokeWeight === "number" && node.strokeWeight > 0) {
      result2.scanned++;
...
async function alignRecursive(node, textOnly, result2, spacingOnly = false) {
  if ("visible" in node && !node.visible) return;
  await alignNode(node, textOnly, result2, spacingOnly);
  if (node.type === "INSTANCE") return;
  if ("children" in node) {
    for (const child of node.children) {
      await alignRecursive(child, textOnly, result2, spacingOnly);
    }
  }
}
async function runFullAlign(node, skipSizing = false) {
  if (!isLoaded()) await loadLibraryTokens();
  let mode = "n/a";
  if (!skipSizing && "layoutMode" in node) {
    mode = await setResponsiveMode(node);
  }
  if (skipSizing) mode = "skipped";
...
async function runTextColorsAlign(node, skipSizing = false) {
  if (!isLoaded()) await loadLibraryTokens();
  let mode = "n/a";
  if (!skipSizing && "layoutMode" in node) {
    mode = await setResponsiveMode(node);
...
async function runSpacingAlign(node, skipSizing = false) {
  if (!isLoaded()) await loadLibraryTokens();
  let mode = "n/a";
  if (!skipSizing && "layoutMode" in node) {
    mode = await setResponsiveMode(node);
  ...
  if (skipSizing) mode = "skipped";
  const result2 = { aligned: 0, scanned: 0, unmatched: [] };
  await alignRecursive(node, false, result2, true);
  return __spreadProps(__spreadValues({}, result2), { mode });
}
...
figma.ui.onmessage = async (msg) => {
  switch (msg.type) {
    ...
    case "full-align-s2a": {
      ...
      try {
        const result2 = await runFullAlign(sel[0], !!msg.skipSizing);
    ...
    case "text-colors-align": {
      ...
      try {
        const result2 = await runTextColorsAlign(sel[0], !!msg.skipSizing);
    ...
    case "spacing-align": {
      ...
      figma.notify("Aligning spacing to S2A library...");
      try {
        const result2 = await runSpacingAlign(sel[0], !!msg.skipSizing);
        figma.notify(`Spacing: ${result2.aligned} bound to S2A, ${result2.unmatched.length} unmatched (mode: ${result2.mode})`);
        figma.ui.postMessage(__spreadValues({ type: "s2a-align-result" }, result2));
      } catch (e) {
        figma.notify(`Spacing Align failed: ${e.message}`, { error: true });
      }
      break;
    }
    case "apply-grid": {
    ...
    case "apply-grid-margin-16": {
      ...
      try {
        ...
        figma.notify("16px margin applied");
        figma.ui.postMessage({ type: "grid-result", message: "Applied 16px margin overlay" });
      } catch (e) {
        figma.notify(`16px margin failed: ${e.message}`, { error: true });
        figma.ui.postMessage({ type: "grid-result", message: `Error: ${e.message}` });
      }
      break;
    }
    case "apply-grid-margin-24": {
      const sel = figma.currentPage.selection;
      if (sel.length === 0) {
        figma.notify("Select a frame first");
        break;
      ...
      try {
        ...
        frame.layoutGrids = [...frame.layoutGrids, grid];
        figma.notify("24px margin applied");
        figma.ui.postMessage({ type: "grid-result", message: "Applied 24px margin overlay" });
      } catch (e) {
        figma.notify(`24px margin failed: ${e.message}`, { error: true });
        figma.ui.postMessage({ type: "grid-result", message: `Error: ${e.message}` });
      }
      break;
    }
    ...
    case "force-match": {
      ...
      try {
        const result2 = await forceMatch(sel[0], categories, !!msg.skipSizing);
    ...
    case "clear-annotations": {
      let clearRecursive2 = function(node) {
        var _a;
        if ("annotations" in node && ((_a = node.annotations) == null ? void 0 : _a.length) > 0) {
          node.annotations = [];
          cleared++;
        }
        if ("children" in node) {
          for (const child of node.children) clearRecursive2(child);
        }
      };
      var clearRecursive = clearRecursive2;
      const sel = figma.currentPage.selection;
      if (sel.length === 0) {
        figma.notify("Select a node first");
        break;
      ...
      let cleared = 0;
      clearRecursive2(sel[0]);
      figma.notify(`Cleared annotations from ${cleared} node${cleared !== 1 ? "s" : ""}`);
      figma.ui.postMessage({ type: "spec-it-status", message: `Cleared ${cleared} annotation${cleared !== 1 ? "s" : ""}` });
      break;
    }
