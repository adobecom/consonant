...
Guidelines:
- Mirrors matt-atoms component set `.ElasticCard` (node `4006-461133`). Keep `data-state="resting|expanded|mobile"` in markup so CSS variants match the Figma axes.
...
- Header must use the shared `ProductLockup` component; `ElasticCard` wires this up automatically, but if you roll custom markup make sure icon sizes follow the `md` (24px) spec and flip `context` when you step into `expanded` / `mobile` states.
...
- For dense routers on desktop, stack one `ElasticCard` (`state="expanded"`) next to two `Card` / `RouterMarqueeItem` instances. Mobile screens typically swap to a single `state="mobile"` card per view.
