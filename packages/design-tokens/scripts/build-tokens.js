...
async function buildFromFigma() {
  ...
  const baseCssPath = path.join("dist", "packages", "design-tokens", "css", "tokens-base.css");
  ...
  for (const [modeSlug, entries] of responsiveModes.entries()) {
    ...
    await filterResponsiveCss(
      path.join("dist", "packages", "design-tokens", "css", tempFile),
      path.join("dist", "packages", "design-tokens", "css", `tokens-${modeSlug}.css`),
    );
  }
...
async function buildCssFromTokensWithMediaQuery(
  ...
  const tempPath = path.join("dist", "packages", "design-tokens", "css", tempFile);
  ...
  const finalPath = path.join("dist", "packages", "design-tokens", "css", destination);
