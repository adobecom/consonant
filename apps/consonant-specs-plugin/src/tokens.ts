...
export async function forceMatch(
  ...
  if (!skipSizing) {
    // Use the top-level frame's width for breakpoint, not the selected node's width
    const topFrame = findTopLevelFrame(node);
    const modeTarget = topFrame || (('layoutMode' in node) ? node as FrameNode : null);
    if (modeTarget) {
