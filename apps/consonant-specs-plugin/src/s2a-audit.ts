...
async function alignNode(node: SceneNode, textOnly: boolean, result: { aligned: number; scanned: number; unmatched: AuditIssue[] }, spacingOnly = false): Promise<void> {
  ...
  if (!textOnly && !spacingOnly && 'fills' in node && Array.isArray((node as any).fills)) {
  ...
  if (!textOnly && !spacingOnly && 'strokes' in node && Array.isArray((node as any).strokes)) {
...
async function alignRecursive(node: SceneNode, textOnly: boolean, result: { aligned: number; scanned: number; unmatched: AuditIssue[] }, spacingOnly = false): Promise<void> {
  if ('visible' in node && !node.visible) return;
  await alignNode(node, textOnly, result, spacingOnly);
  // Don't descend into instance children — they're controlled by the component
  if (node.type === 'INSTANCE') return;
  if ('children' in node) {
    for (const child of (node as any).children) {
      await alignRecursive(child, textOnly, result, spacingOnly);
    }
  }
}

export async function runFullAlign(node: SceneNode, skipSizing = false): Promise<AlignResult> {
  if (!isLoaded()) await loadLibraryTokens();
  let mode = 'n/a';
...
export async function runTextColorsAlign(node: SceneNode, skipSizing = false): Promise<AlignResult> {
  if (!isLoaded()) await loadLibraryTokens();
  let mode = 'n/a';
  if (!skipSizing && 'layoutMode' in node) {
    mode = await setResponsiveMode(node as FrameNode);
...
export async function runSpacingAlign(node: SceneNode, skipSizing = false): Promise<AlignResult> {
  if (!isLoaded()) await loadLibraryTokens();
  let mode = 'n/a';
  if (!skipSizing && 'layoutMode' in node) {
    mode = await setResponsiveMode(node as FrameNode);
  ...
  if (skipSizing) mode = 'skipped';
  const result = { aligned: 0, scanned: 0, unmatched: [] as AuditIssue[] };
  await alignRecursive(node, false, result, true);
  return { ...result, mode };
}
